...
file_handler = logging.FileHandler(
    filename=f"{log_directory}/app_{current_date}_{current_time}.log"
)
...
def load_nvd_cve_data(start_year: int, end_year: int, directory: str) -> pd.DataFrame:
    ...
    for file in os.listdir(directory):
        ...
        if file.endswith(".json") and int(file.split("-")[2].split(".")[0]) in range(
                    ...
                    if cve_data.get("impact"):
                        for key in cve_data.get("impact"):
                            if key == "baseMetricV3":
                                ...
                                last_modified_date = cve_data.get(
                                    "lastModifiedDate", ""
                                )
                                problemtype_data = (
                                    cve_data.get("cve", {})
...
def download_epss_scores_for_months(months, directory):
    ...
    for i in range(months):
        date = (
            datetime.strptime(today_date, "%Y-%m-%d").replace(day=1) - timedelta(days=1)
        ).replace(day=1)
    ...
    if os.path.exists(file_path):
        ...
        with open(file_path) as file:
            ...
            if cve.get("cveMetadata", {}).get("state") != "REJECTED":
                ...
                position = get_metric_position_of_other(
                    adp_list[adp_position].get("metrics", {})
                )
...
def update_row_with_details(row):
    details = flatten_vulnrichment_output(
        cve_vulnrichment(row["cve_id"])
    )  # Fetch details for the current row's cve_id
    if not details:
        return
...
def get_cwe_name_and_description(cwe_id):
    try:
        ...
        if (
            cwe_id in ["not_found", "NVD-CWE-noinfo", "NVD-CWE-Other"]
            or not cwe_id
            or cwe_id == ""
        ):
            return {
        ...
        cwe_name = (
            cwe_data.get("Weaknesses")[0].get("Name")
            if cwe_data.get("Weaknesses")[0].get("Name")
            else "not_found"
        )
        cwe_description = (
            cwe_data.get("Weaknesses")[0].get("Description")
            if cwe_data.get("Weaknesses")[0].get("Description")
            else "not_found"
        )
        cwe_cc_impact = (
            list_to_csv(
                cwe_data.get("Weaknesses")[0].get("CommonConsequences")[0].get("Impact")
            )
            if cwe_data.get("Weaknesses")[0].get("CommonConsequences")
            else "not_found"
        )
        cwe_cc_scope = (
            list_to_csv(
                cwe_data.get("Weaknesses")[0].get("CommonConsequences")[0].get("Scope")
            )
            if cwe_data.get("Weaknesses")[0].get("CommonConsequences")
            else "not_found"
        )
