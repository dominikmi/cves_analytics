...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("logs/app_g.log"), logging.StreamHandler()],
)
...
class DockerImageScanner:
    ...
    def __init__(self, registry_url: str = None):
        ...
        logger.info(
            "Initialized DockerRegistryScanner with registry URL: %s", self.registry_url
        )
    ...
    def scan_image_with_grype(self, image: str) -> pd.DataFrame:
        ...
        try:
            ...
            if not self.registry_url:
                ...
                grype_results = json.loads(
                    self.grype.scan(
                        self.registry_url.split("//")[1] + "/" + image
                    ).to_json()
                )
            logger.info(
                "Grype scan results: image: %s, found: %s vulnerabilities",
                image,
                len(grype_results.get("matches", [])),
            )
            vulnerabilities = []
            if len(grype_results.get("matches", [])):
                    ...
                    vulnerabilities.append(
                        {
                            ...
                            "fixed_ver": vuln.get("matchDetails")[0]
                            .get("found")
                ...
                return df.sort_values(by=["severity"], ascending=True).reset_index(
                    drop=True
                )
            else:
