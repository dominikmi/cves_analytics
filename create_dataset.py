...
def main():
    ...
    # save the data to a CSV file
    cves_with_epss.to_csv(
        f"{args.output_path}/cves_with_epss_{args.start_year}-{args.end_year}_{today_date}.csv",
        index=False,
    )
    ...
    cves_with_kevs = pd.merge(
        cves_with_epss, kevs, left_on="cve_id", right_on="cveID", how="left"
    )

    logger.info("Processing the data for KEVs")
    cves_with_kevs = cves_with_kevs.apply(
        lambda row: update_row_with_details(row)
        if isinstance(row["cve_id"], str)
        else None,
        axis=1,
    )

    # normalize description data, some are with "" some not
    cves_with_kevs["description"] = cves_with_kevs["description"].apply(
        lambda x: x if x else ""
    )

    # filter out all with Rejected CVEs in description
    cves_with_kevs = cves_with_kevs[
        ~cves_with_kevs["description"].str.contains("Rejected reason:")
    ]
    ...
    list_of_cwes = list(
        set(
            cves_with_kevs[
                ~cves_with_kevs["cwe_id"].isin(
                    ["not_found", "NVD-CWE-noinfo", "NVD-CWE-Other"]
                )
            ]["cwe_id"].to_list()
        )
    )
    cwes_df = pd.DataFrame(
        [get_cwe_name_and_description(cwe_id) for cwe_id in list_of_cwes]
    )
    cve_final = cves_with_kevs.merge(cwes_df, on="cwe_id", how="left")
    cve_final = cve_final.fillna("not_found")

    # organizing the columns
    cve_final = cve_final.rename(
        columns={
            "vendorProject": "kev_vendor_proj",
            "product": "kev_product",
            "vulnerabilityName": "kev_vuln_name",
            "dateAdded": "kev_date_added",
            "shortDescription": "kev_desc",
            "requiredAction": "kev_req_action",
        }
    )
    cve_final = cve_final.rename(
        columns={
            "dueDate": "kev_due_date",
            "knownRansomwareCampaignUse": "kev_known_ransom_camp_use",
            "cwes": "kev_cwes",
            "Exploitation": "cve_exploitable",
            "Automatable": "cve_automatable",
            "Technical Impact": "cve_tech_impact",
        }
    )
    ...
    # save the data to a CSV file
    cve_final.to_csv(
        f"{args.output_path}/cves_epss_kevs_cwe_enriched-{today_date}.csv", index=False
    )
