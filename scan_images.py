...
logging.basicConfig(
    filename=os.path.join(log_directory, "scan_images.log"),
    level=logging.INFO,
...
parser.add_argument(
    "-l", "--list_of_images", required=False, help="Docker image list in CSV format"
)
parser.add_argument(
    "-t",
    "--tls",
...
parser.add_argument(
    "-o", "--output", required=True, help="Output CSV file path for scan results"
)
parser.add_argument(
    "-e",
    "--enrich_with_data",
    required=False,
    help="Enrich with CVE data, path to CSV file",
)
...
def main():
    ...
    if args.list_of_images:
        try:
            ...
            logging.error(
                f"Error scanning images from list: {args.list_of_images}, error: {e}"
            )
            return
    ...
    if not args.enrich_with_data:
        ...
        if len(results):
            try:
                logging.info(
                    f"Enriching scan results with CVE data from: {args.enrich_with_data}"
                )
                cve_data = pd.read_csv(args.enrich_with_data)
                enriched_results = pd.merge(
                    results, cve_data, left_on="vuln_id", right_on="cve_id", how="left"
                )

                # make all colunm name lowercase
                enriched_results.columns = enriched_results.columns.str.lower()
                enriched_results = enriched_results.drop(
                    columns=["cve_id", "description"]
                )
