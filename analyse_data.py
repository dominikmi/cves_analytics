...
class VulnerabilityAnalyzer:
    ...
    def predict_risk(self):
        ...
        for _ in range(total_years):
            year = np.random.choice(
                list(self.analyzer.historical_data["year"]), replace=False
            )
            type_ = self.analyzer.analyzer.predicted_vulnerability(year).index[0]

            risk_level = (
                (
                    self.analyzer.analyzer.predicted_vulnerability(year)[type_]
                    / sum(self.analyzer.analyzer.predicted_vulnerability(year))
                    * 100
                )
                if type_
    ...
    def run_simulation(self):
        results = {"year": range(2020, 2025), "predicted_risk": self.predict_risk()}
        return results
...
def download_csv():
    if request.method == "POST":
        ...
        # Export as CSV.
        output_file_name = (
            f"vulnerability_analysis_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
        )
